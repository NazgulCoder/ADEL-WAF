...
class AdelWAF {
	...
	private $xssRules = array("<img", "img>", "<image", "</scr", "<script", "javascript:", "data:", "vbscript:", "<svg", "alert(", "document.cookie", "document.location", "eval(", "base64_encode", "base64_decode", "expression(", "<iframe", "srcdoc=", "onabort=","onauxclick=","oncancel=","oncanplay=","oncanplaythrough=","onchange=","onclick=","onclose=","oncontextmenu=","oncuechange=","ondblclick=","ondrag=","ondragend=","ondragenter=","ondragexit=","ondragleave=","ondragover=","ondragstart=","ondrop=","ondurationchange=","onemptied=","onended=","onformdata=","oninput=","oninvalid=","onkeydown=","onkeypress=","onkeyup=","onloadeddata=","onloadedmetadata=","onloadstart=","onmousedown=","onmouseenter=","onmouseleave=","onmousemove=","onmouseout=","onmouseover=","onmouseup=","onpause=","onplay=","onplaying=","onprogress=","onratechange=","onreset=","onsecuritypolicyviolation=","onseeked=","onseeking=","onselect=","onslotchange=","onstalled=","onsubmit=","onsuspend=","ontimeupdate=","ontoggle=","onvolumechange=","onwaiting=","onwebkitanimationend=","onwebkitanimationiteration=","onwebkitanimationstart=","onwebkittransitionend=","onwheel=","onblur=","onerror=","onfocus=","onload=","onresize=","onscroll=","onafterprint=","onbeforeprint=","onbeforeunload=","onhashchange=","onlanguagechange=","onmessage=","onmessageerror=","onoffline=","ononline=","onpagehide=","onpageshow=","onpopstate=","onrejectionhandled=","onstorage=","onunhandledrejection=","onunload=","oncut=","oncopy=","onpaste=","onreadystatechange=","<frame","<embed","<object","href=","src=");
	
	private $sqliRules = array("table_schema", "or '1", "unhex(hex(concat(", "select*from", ";--","and(select", "or(select", "count(", "information_schema", "schema_name", "extractvalue", "concat(", "json_keys(", "droptable", "selectif","'select","unionall","'and'","'or'", "unionselect","orderby","groupby","insertinto","intooutfile","benchmark(","waitfordelay","waitfortime","sleep(", "select", "insert", "update", "delete", "drop", "alter", "truncate", "create", "grant", "revoke", "union", "from", "into", "where", "table", "index", "user", "procedure", "function", "database", "exec", "xp_cmdshell", "' and ", "' or ", "-- -", "/* */");
	
	private $lfiRules = array("../", "..", "../", "%2e%2e/", "%2e%2e%5c", "%252e%252e", "etc/passwd", "etc/shadow", "etc/hosts", "proc/self/environ", "boot/grub/grub.cfg", "/etc/", "/proc/", "/sys/", "/boot/");
	
	private $rfiRules = array("file://","ftp://","ftps://","http://","https://", "gopher://", "dict://", "ldap://", "tftp://");
	
    private $rceRules = array("bin/", "cmd/", "&&", ">/", "system(","exec(", "<?", "?>", "cat ", "base64 ", "whoami", "echo ", "curl ", "wget ", "bash ", "sh ", "python ", "perl ", "ls ", "id ", "ping ", "nslookup", "ipconfig", "ifconfig", "powershell");
	
	private $scannerRules = array("nikto", "sqlmap", "nmap", "acunetix", "nessus", "openvas", "wpscan", "dirbuster", "burpsuite", "owasp zap", "netsparker", "appscan", "arachni", "skipfish", "gobuster", "wfuzz", "hydra", "metasploit", "qualys", "w3af", "nuclei", "zgrab", "vega", "gospider", "whatweb", "joomscan", "uniscan", "blindelephant");
	
	private $legitimateBrowsers = array("Mozilla", "Chrome", "Safari", "Edge", "Firefox", "Opera", "AppleWebKit", "Gecko", "Trident", "MSIE", "Googlebot", "Bingbot", "Slurp", "DuckDuckBot", "Baiduspider", "YandexBot", "facebot", "Twitterbot", "LinkedInBot");
	
	private $crlfRules = array("%0d%0a", "%0a%0d", "\\r\\n", "%0d", "%0a", "\\r", "\\n");
	
	private $nosqlRules = array('$gt', '$gte', '$lt', '$lte', '$ne', '$eq', '$regex', '$where', '$or', '$and', '$in', '$nin', '$exists', '$type', '$not', '$mod', '$elemMatch', '$all', '$size', '$nor', '$comment', '$slice', '$expr', '$meta', '$text', '$search', '$near', 'db.', 'collection.', 'aggregate(', 'mapReduce(', 'findOne(', 'find(', 'remove(', 'update(', 'insert(');
	
	// private $ssrfRules = array("127.0.0.1", "localhost", "10.", "172.", "192.168.", "169.254.", "0.0.0.0", "::1", "metadata", "aws", "digitalocean", "google", "azure", "internal", "local", "intranet", "test");
	
	private $sstiRules = array("{{", "}}", "{%", "%}", "$\{", "#}", "$(", ")", "{*", "*}", "#*", "*#", "<%", "%>", "@{", "Runtime", "Process", "getClass", "ClassLoader", "loadLibrary", "forName", "newInstance", "getMethod", "invoke", "getConstructor", "readObject", "writeObject");
	
	private $sensitiveFiles = array(".git/", ".env", ".htaccess", ".htpasswd", ".svn/", ".DS_Store", "WEB-INF/", "META-INF/", ".dockerenv", "server-status", "web.xml", "wp-config.php", "config.json", "database.yml", "credentials", "secrets", "private", "backup", ".bak", ".swp", ".orig", ".dist", ".sample", ".example", ".template", ".sql");
	
	// private $unusualPaths = array("/wp-admin", "/phpmyadmin", "/admin", "/login", "/cgi-bin", "/shell", "/backdoor", "/cmd", "/exec", "/bin/sh", "/bin/bash", "/console", "/setup", "/test", ".php", ".asp", ".aspx", ".jsp", ".do", ".action", ".pl", ".py", ".cgi", ".cfm", ".rb", ".phtml");
	
	private $redirectRules = array("redirect=http", "url=http", "next=http", "return=http", "r=http", "u=http", "redirect=//", "url=//");
	
	private $idorRules = array("id=", "user=", "account=", "profile=", "order=", "item=", "product=", "comment=", "post=", "blog=", "thread=", "task=", "note=", "group=", "file=", "image=", "report=", "json=", "api=", "download=", "admin=", "dashboard=", "email=", "video=");
	
	private $deserializationRules = array("rO0AB", "aced0005");
	
	private $jwtRules = array("eyJ", "Bearer eyJ");
	
	private $smugglingRules = array("Transfer-Encoding", "Content-Length: 0", "TE: chunked", "TE: identity");
	
	private $webShellRules = array("<h1 style=\"margin-bottom: 0\">webadmin.php</h1>", "b374k m1n1", "Shell I</title>", "SmEvK_PaThAn Shell", "by C-W-M</title>", "<title>azrail ", "<title>punkholicshell</title>", "<title>g00nshell", "<title>PhpSpy Ver", "Cr@zy_King</title>", "<title>s72 Shell v", "<title>Ru24PostWebShell -", "<font size=\"1\">Input command :</font>", "<title>PHP Web Shell</title>", "<title>lostDC - ", "<title>lama's'hell v", "<title>Web Shell</title>", "<title>SimAttacker - ", "<small>NGHshell", "<title>GRP WebShell ", "<title>CasuS ", "by MafiABoY</title>", "<title>Symlink_Sa", "Ashiyane V", "Developed By LameHacker", "<title>Mini Shell</title>", "- WSO","B4TM4N SH3LL</title>", "<meta name='author' content='k4mpr3t'/>", "<title>=[ 1n73ct10n privat shell ]=</title>", ">Ajax/PHP Command Shell<", ".:: :[ AK-74 Security Team Web-shell ]: ::.", "~ ALFA TEaM Shell -", "<title>--==[[ Andela Yuwono Priv8 Shell ]]==--</title>", "<title>Ani-Shell | India</title>", "<input type='submit' value='file' /></form>AnonymousFox", "- Antichat Shell</title>", "Ayyildiz Tim  | AYT", "<title>BloodSecurity Hackers Shell</title>", "<font color='red' size='6px' face='Fredericka the Great'> Bypass Attack Shell </font>", "title='.::[c0derz shell]::.'>", "<font face=Webdings size=6><b>!</b></font>", "<title>Con7ext Shell V.2</title>", "<font face=\"Wingdings 3\" size=\"5\">y</font><b>Crystal shell v.", "~ CWShell ~</font></a>", "&dir&pic=o.b height= width=>", "<b>[ Defacing Tool Pro v", "<title>Dive Shell - Emperor Hacking Team</title>", "<script>document.getElementById(\"cmd\").focus();</script>", "<p align=\"center\" class=\"style4\">FaTaLSheLL v", "<title>G-Security Webshell</title>", "<title>h4ntu shell [powered by tsoi]</title>", "<H1><center>-=[+] IDBTEAM SHELLS", "<title>IndoXploit</title>", "<KAdot Universal Shell>     |", ">LIFKA SHELL</span></big></big></big></a>", "<title>Loader'z WEB shell</title>", "b>--[ x2300 Locus7Shell v.", "<title>Lolipop.php - Edited By KingDefacer -", "<title> Matamu Mat </title>", "<b>MyShell</b> &copy;2001 Digitart Producciones</a>", "<h1>.:NCC:. Shell v", "<font size=3>PHPShell by Macker - Version", "PHPShell by MAX666, Private Exploit, For Server Hacking", "<form action=\"\" METHOD=\"GET\" >Execute Shell Command (safe mode is off): <input type=\"text\" name=\"c\"><input type=\"submit\" value=\"Go\"></form>", "<p align=\"center\"><font face=\"Verdana\" size=\"2\">Rootshell v", "<font color=lime>./rusuh</font>", "<center><h1>Watch Your system Shany was here.</h1></center><center><h1>Linux Shells</h1></center><hr><hr>", "<!-- Simple PHP backdoor by DK", "<title>SimShell - Simorgh Security MGZ</title>", "<title>:: AventGrup ::.. - Sincap", "<title>Small Shell - Edited By KingDefacer</title>", "<title>small web shell by zaco", "<title>SoldiersofAllah Private Shell |", "<title>Sosyete Safe Mode Bypass Shell", "&nbsp;&nbsp;STNC&nbsp;WebShell&nbsp;", "<font face=\"Wingdings 3\" size=\"5\">y</font><b>StresBypass<span lang=\"en-us\">v", "<title>SyRiAn Sh3ll ~", "<head><title>Wardom | Ne Mutlu T", "<hr>to browse go to http://?d=[directory here]", "<font color=\"red\">USTADCAGE_48</font> <font color=\"dodgerblue\">FILE MANAGER</font>", "<div align=\"center\"><span class=\"style6\">By BLaSTER</span><br />", "<title>-:[GreenwooD]:- WinX Shell</title>", "<title>Yourman.sh Mini Shell</title>", "</div><center><br />Zerion Mini Shell <font color=", "<title>0byt3m1n1-V2</title>", "<title>ZEROSHELL | ZEROSTORE</title>", "<input type=submit name=find value='find writeable'>", "<title>r57 shell</title>");
	...
	function getHost() {
		$host = $this->getEnvir('HTTP_HOST');
		if ($host === 'UNKNOWN') {
			$host = $this->getEnvir('SERVER_NAME');
			if ($host === 'UNKNOWN') {
				return 'localhost';
			}
		}
	...
	function checkHeaders($info) {
		...
		foreach ($headers as $name => $value) {
			...
			if ($this->striposa($this->crlfRules, $headerString)) {
				$this->warn($info, 'CRLF injection attempt', $name, $value);
			}
			
			if ($this->striposa($this->sqliRules, $headerString)) {
				$this->warn($info, 'SQL injection in headers', $name, $value);
			}
			
			if ($this->striposa($this->xssRules, $headerString)) {
				$this->warn($info, 'XSS in headers', $name, $value);
			}
			
			if ($this->striposa($this->smugglingRules, $headerString)) {
				$this->warn($info, 'HTTP request smuggling', $name, $value);
			}
			
			if (strtolower($name) === 'authorization' && $this->striposa($this->jwtRules, $value)) {
				$this->warn($info, 'JWT tampering attempt', $name, $value);
			}
		}
	}
	...
	function checkUserAgent($info) {
		...
		if ($this->striposa($this->scannerRules, strtolower($userAgent))) {
			$this->warn($info, 'Security scanner detected', 'User-Agent', $userAgent);
		}
		
		if (!$this->striposa($this->legitimateBrowsers, $userAgent)) {
			$this->logs($info, 'Suspicious User-Agent', 'User-Agent', $userAgent);
		}
	}
	
	function checkURI($info) {
		$uri = $_SERVER['REQUEST_URI'] ?? '';
		$uri = strtolower($uri);
		
		if ($this->striposa($this->lfiRules, $uri)) {
			$this->warn($info, 'Path traversal attempt', 'URI', $uri);
		}
		
		if ($this->striposa($this->sensitiveFiles, $uri)) {
			$this->warn($info, 'Sensitive file access', 'URI', $uri);
		}
		
		/*
		if ($this->striposa($this->ssrfRules, $uri)) {
			$this->warn($info, 'SSRF attempt', 'URI', $uri);
		}
		*/
	}
	...
	function checkPostData($info) {
		...
		if ($this->striposa($this->nosqlRules, $postData)) {
			$this->warn($info, 'NoSQL injection attempt', 'POST_DATA', substr($postData, 0, 100));
		}
		
		if ($this->striposa($this->sstiRules, $postData)) {
			$this->warn($info, 'Server-Side Template Injection', 'POST_DATA', substr($postData, 0, 100));
		}
		
		if ($this->striposa($this->deserializationRules, $postData)) {
			$this->warn($info, 'Insecure deserialization', 'POST_DATA', substr($postData, 0, 100));
		}
		
		if ($this->striposa($this->redirectRules, $postData)) {
			$this->warn($info, 'Open redirect attempt', 'POST_DATA', substr($postData, 0, 100));
		}
	}
	
	function checkCookies($info) {
		if (empty($_COOKIE)) return;
		
		foreach ($_COOKIE as $name => $value) {
			$cookieData = strtolower($name . '=' . $value);
			
			if ($this->striposa($this->jwtRules, $value)) {
				$this->warn($info, 'JWT in cookie', $name, $value);
			}
			
			if ($this->striposa($this->nosqlRules, $cookieData)) {
				$this->warn($info, 'NoSQL injection in cookie', $name, $value);
			}
			
			if ($this->striposa($this->sstiRules, $cookieData)) {
				$this->warn($info, 'SSTI in cookie', $name, $value);
			}
		}
	}
	...
	function sanitizeAndCheckAttacks($info, $key, $value) {
		...
		elseif ($this->striposa($this->nosqlRules, $sanitizedValue))
			$this->warn($info, 'NoSQL injection', $key, $sanitizedValue);
		/*
		elseif ($this->striposa($this->ssrfRules, $sanitizedValue))
			$this->warn($info, 'Server-Side Request Forgery (SSRF)', $key, $sanitizedValue);
		*/
		elseif ($this->striposa($this->sstiRules, $sanitizedValue))
			$this->warn($info, 'Server-Side Template Injection (SSTI)', $key, $sanitizedValue);
		elseif ($this->striposa($this->redirectRules, $sanitizedValue))
			$this->warn($info, 'Open redirect attempt', $key, $sanitizedValue);
		elseif ($this->striposa($this->idorRules, $sanitizedValue))
			$this->warn($info, 'IDOR attempt', $key, $sanitizedValue);
		return;
	}
	...
	function run() {
		$userAgent = $this->getEnvir('HTTP_USER_AGENT');
		if (strpos($userAgent, 'health-checker') !== false) {
			return;
		}
		...
		if ($this->ENABLE_WAF) {	
			...
			if (!$this->striposa($this->EXCLUDE_DOMAINS, $this->getHost())) {
				
				$this->checkUserAgent($info);
				$this->checkHeaders($info);
				$this->checkURI($info);
				$this->checkPostData($info);
				$this->checkCookies($info);
				
				if (count($_REQUEST) > $this->MAX_REQ_LEN) {
					$this->warn($info, 'Denial of service (DOS)', 'count', count($_REQUEST));
				} else {
					if (!isset($_COOKIE['phpMyAdmin'])) {
						$currentFile = ($_SERVER['DOCUMENT_ROOT'] ?? '/var/www/html') . 
									   ($_SERVER['PHP_SELF'] ?? '/index.php');
						
						if (file_exists($currentFile) && is_readable($currentFile)) {
							if ($this->striposa($this->webShellRules, file_get_contents($currentFile))) {
								$this->warn($info, 'Web shell detected');
							}
						}
